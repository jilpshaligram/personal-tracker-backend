...
import { ISavingTransaction } from '../interfaces/saving-transaction.interface';
...
import { SAVING_TRANSACTION_QUERY_FIELDS } from '../constants/query-fields.constants';
...
export class SavingTransactionService {
  ...
  async create(
    ...
    // 1. Verify the parent goal — throws if not found or not owned by user
    const goal = await this.savingGoalService.findGoalOrFail(goalId, userId);

    // 2. Guard against overdraft for WITHDRAWAL
    ...
    return this.sequelize.transaction(async (t) => {
      ...
      // 4. Recalculate the parent goal's totals atomically
  ...
  async update(
    ...
    // Need goal details for overdraft checking if switching to WITHDRAWAL or increasing WITHDRAWAL
    ...
    if (newType === SavingTransactionType.WITHDRAWAL) {
      // Calculate what the savedAmount would be WITHOUT this existing transaction
      const currentSaved = Number(goal.savedAmount);
    ...
    return this.sequelize.transaction(async (t) => {
      ...
      // if (dto.note !== undefined) transactionRecord.note = dto.note;
  ...
  async remove(id: string, userId: string): Promise<void> {
    ...
    // Run delete and recalculation atomically
    await this.sequelize.transaction(async (t) => {
      // Soft-delete the record
      await transaction.destroy({ transaction: t });

      // Recalculate the parent goal now that the transaction is gone
      await this.recalculateSavingGoal(goalId, t);
    });
  }
  ...
  private async recalculateSavingGoal(
    ...
    // Sum all CONTRIBUTION amounts within this transaction
    const contributionResult = (await this.savingTransactionModel.findOne({
      ...
      raw: true,
      transaction, // Include in DB transaction
    })) as unknown as { total: string };

    // Sum all WITHDRAWAL amounts within this transaction
    ...
    // Delegate update to SavingGoalService, passing the transaction
